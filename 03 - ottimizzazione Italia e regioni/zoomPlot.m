...
hold on
% Piccola correzione: for alpha= 0 to 1 the lines will be more clear
alpha = 0.75;
if any(vertex==1)
    plot([xbounds(1) x1], [max(y(index)) y2], 'color',[0,0,0]+alpha); % Line to vertex 1
end
if any(vertex==2)
    plot([xbounds(2) x2], [max(y(index)) y2], 'color',[0,0,0]+alpha); % Line to vertex 2
end
if any(vertex==3)
    plot([xbounds(2) x2], [min(y(index)) y1], 'color',[0,0,0]+alpha); % Line to vertex 4
end
if any(vertex==4)
    plot([xbounds(1) x1], [min(y(index)) y1], 'color',[0,0,0]+alpha); % Line to vertex 3
end
